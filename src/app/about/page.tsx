...
const About = () => {
  const data: SectionsProp = getListPage("about/_index.md");
        ...
        <div className="container">
          <div className="grid gap-36">
            {sections.map((section, index) => {
              return (
                <div
                  className={`flex flex-col gap-24 items-center ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}
                  key={section.title}
                >
                  {/* LEFT */}
                  <div className="md:w-1/2 w-full">
                    ...
                    />
                    <p
                      dangerouslySetInnerHTML={markdownify(section.description)}
                    />
                  </div>
                  {/* RIGHT */}
                  <div className="md:w-1/2 w-full">
