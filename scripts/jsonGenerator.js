...
const RESOURCE_FOLDER = "src/content/resources";
...
try {
  ...
  fs.writeFileSync(
    `${JSON_FOLDER}/resources.json`,
    JSON.stringify(getData(RESOURCE_FOLDER, 2)),
  );

  // merger json files for search
  const posts = require(`../${JSON_FOLDER}/posts.json`);
  const resources = require(`../${JSON_FOLDER}/resources.json`);
  const search = [...resources, ...posts];
